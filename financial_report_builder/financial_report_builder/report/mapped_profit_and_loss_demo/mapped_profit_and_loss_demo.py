...
from financial_report_builder.financial_report_builder.doctype.report_schema.report_schema_view import get_nodes
...
def execute(filters=None):
	...
	nodes, report_sources= get_nodes('Mapped Profit and Loss Schema') # get nodes docs from the report schema
...
def get_data(period_list: list, filters: dict, nodes: list, report_sources: set):
	...
	for node in nodes:
		...
		if node.row_based_on_formula:
			...
			row.update({'account': node.label,'account_name': node.label, 'label': node.label, 'indent': 0})
